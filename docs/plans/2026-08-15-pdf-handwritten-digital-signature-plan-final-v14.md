...
**版本：** v14 Final Lean（在 v13 基础上补齐取消与私钥边界仲裁、completed result 完整性状态及 Java 最小权限合同）

**状态：** **静态方案复审 GO，无阻塞性 finding。** Phase S 可立即实施；Phase 0A 通过后进入 Phase 1；Phase 0B 通过后进入三人顺序签署与 deferred 首页章。Gate S、Gate 0、真实 Chrome、Acrobat、Foxit、独立 PAdES 验证和故障注入仍是实现与发布证据门禁，不代表这些运行时证据已经取得。本版 GO 已包含对 v13 外部复审提出的 1 个 P0、2 个 P1 的正文修复及取消/完成结果恢复的反向竞态复核。
...
- `publication_version BIGINT UNSIGNED DEFAULT 0`；
- `integrity_state`: `ok` / `hold`；nullable `integrity_hold_reason`, `integrity_hold_started_at`；
...
- `evidence_hold_state`: `none` / `active`；nullable `evidence_hold_reason`: `manual_review` / `irreversible_failure` / `quarantine`；
- nullable `claimed_by_operation_id`, `retention_until`, `legal_hold_until`, `hold_started_at`, `hold_released_at`, `deleted_at`, `file_path`, timestamps。
...
challenge 创建后只接受 `appearance_uuid`，不再接收替换图片。

保留与清理合同：

1. 普通成功或可证明发生在私钥前的终态失败，`evidence_hold_state=none`，`retention_until=terminal_at+24h`；
2. `failed_after_private_key_known`、`outcome_unknown/manual_review` 或关联 quarantine 产物必须在同一终态事务 CAS `evidence_hold_state=none→active`，写 hold reason/start；结案前不得自动复用或删除；
3. 人工结案事务写 `hold_released_at`，CAS `active→none`，并从 release 时刻重新计算至少 24 小时保留期；任何处置失败都不得释放 hold；
4. sweeper 只有在 `evidence_hold_state=none`、`retention_until IS NOT NULL AND retention_until<=now()`、`legal_hold_until IS NULL OR legal_hold_until<=now()`，且不存在非终态 operation、manual review 或 quarantine 引用时才能删除；
5. 删除成功后才写 `deleted_at/state=expired`；删除失败保持原状态并告警；
6. 长期审计保留 appearance hash、尺寸、规范化版本、operation 归属和成品/隔离 PDF，不能因隐私清理提前破坏人工复核证据。
...
- nullable `signing_policy_version_id`, nullable `policy_hash`, nullable `config_bundle_hash`, nullable `expected_certificate_fingerprint`, nullable `appearance_manifest_hash`；`fill_signature_field` 全部必填，非签署 action 按 action-specific rule 禁止无关字段；
...
- nullable `result_retirement_not_before`, nullable `result_retirement_authorized_at`, nullable `result_retirement_authorization_hash`；这些字段只能由 Laravel retention projector 写入/撤销；
...
- nullable `result_last_verified_at`, nullable `result_integrity_error_code`, nullable `retention_until`, nullable `legal_hold_until`, nullable `retirement_started_at`, nullable `bytes_deleted_at`；
- nullable `error_code`, timestamps。

合同：

所有 Java execution 状态 CAS 都必须校验读取时的 exact `lock_version`、`input_fingerprint` 和 `policy_hash`，成功时原子递增 `lock_version`；影响行数不是 1 即退出。每次 attempt 的开始、失败、重试授权、私钥边界和终态都在**同一数据库事务**写 append-only 审计事件，事件 INSERT 失败必须回滚对应状态 CAS；至少记录 attempt number、old/new state、old/new lock version、authorized lease epoch、错误码和时间，避免单行重试覆盖历史证据。
...
- `RESULT_READ_OR_INTEGRITY`：要求 execution 已 `completed`，operation UUID/action/input snapshot 匹配，operation 可处于 `processing/promoted/manual_review/completed`；该 gate 只能读取或推进 result integrity，绝不能创建 attempt、重置 execution 或写私钥边界；
- `RESULT_RETIRE`：要求 operation=`completed`，并核对 Laravel 写入的未撤销 retirement authorization hash/not-before 与 execution result metadata 完全匹配、当前无 legal hold；Java 不直接读取 revision/document 表。
...
9. completed result 每次供 Laravel 取回前都必须先赢得 `RESULT_READ_OR_INTEGRITY` gate CAS，再锁 execution；仅在 `result_integrity_state=available` 时重新核对 path、SHA 和 size、原子写 `result_last_verified_at`。文件不存在置 `missing`；文件存在但 SHA/size 不一致置 `breached`。execution 仍保持 `completed`，以永久阻止重复私钥调用；两种状态都禁止返回 bytes、禁止重新签名，并触发第 7.7 节的下游副本恢复、人工复核或完整性处置；所有状态变化写入现有 append-only execution audit，不另依赖未建模的事故表；
10. Java execution 完成事务先写 `retention_until >= execution.completed_at + 7 days`，因为此时 Laravel operation 可能尚未完成；事务 B 将 operation 置 completed 时，再把 retention 原子延长到 `max(existing_retention_until, operation.completed_at + 7 days)`。manual review、quarantine 或调查中同步写/延长 `legal_hold_until`。Java 无 revision/document 读取权限，因此**不能自行判断正式 revision 是否可替代 execution result**：Laravel `AuthorizeJavaResultRetirementJob` 必须按 operation→execution→document→revision 锁序，验证 operation completed、正式 revision 的 UUID/SHA/签名均正确、保留期已到、无 hold/非终态引用后，才在 operation 写一次性 `result_retirement_not_before/authorized_at/authorization_hash`；hash 覆盖 operation UUID、execution result SHA/size、正式 revision UUID/SHA 和 not-before。任何新增 legal/integrity/manual-review hold 都必须按同一锁序清除此授权。Java result sweeper 只有在 `RESULT_RETIRE` gate CAS 核对该 authorization snapshot、execution result metadata、`now>=not_before` 和当前 `legal_hold_until` 后，才能 CAS `available→retiring` 写 `retirement_started_at`，随后删除 bytes，删除成功再 CAS `retiring→retired` 并写 `bytes_deleted_at`。进程在删除前崩溃且文件仍在时继续删除；删除后、DB 终态前崩溃且文件已不存在时补交 `retired`，不得误报 missing。删除或补交失败保持 `retiring` 并告警；
...
18. V1 部署固定 Java replicas=1；result root 必须是容器生命周期外的持久化 bind volume，启动 health 执行 create/fsync/rename/read-back 探针且失败时签名 readiness fail closed；增加多实例前必须升级到后续完整执行账本方案。
...
- `old_state`, `new_state`, `old_lock_version`, `new_lock_version`, `authorized_lease_epoch`；
...
- operation `java_gate_version` 只能由 Java 账号对该专用列执行条件 UPDATE；`SIGN_AUTHORIZE`、`RESULT_READ_OR_INTEGRITY`、`RESULT_RETIRE` 三类 gate predicate 必须使用不同的服务方法和测试向量，结果读取/退休 gate 永远不得授权创建 attempt 或写私钥边界；`RESULT_RETIRE` 还必须验证 Laravel 持久化的 retirement authorization snapshot，Java 不得读取 revision/document 自行推导；Laravel 取消/事务 B/人工处置锁定同一 operation 行，形成统一 operation→execution 仲裁；
...
- execution `completed` 必须具有 result metadata，且 `result_integrity_state` 只能为 `available/retiring/missing/breached/retired`；非 completed execution 必须为 `not_applicable`。`retired` 永久终态；`missing/breached` 只有受控管理员从可信备份恢复同一 immutable result path、SHA、size并写独立完整性审计后才能 CAS 回 available；`retiring` 只能由授权 sweeper 按第 10 条恢复或终结；V1 不自动重建 execution result；
...
- appearance sweeper 必须同时满足 `evidence_hold_state=none`、retention、legal hold 和无非终态引用，禁止仅按创建时间删除；
...
5. 条件消费 challenge，claim appearance；
...
3. registration deadline 前 execution 尚不可见属于受控观察状态：status 返回 202，Laravel 按 `java_status_poll_policy` 轮询并 heartbeat；execution 一旦可见即转入其 claimed/executing/terminal 状态处理。正常响应或 status 查询取得 completed result 后，必须先确认 `result_integrity_state=available` 并重新核对 result path/SHA/size，才可流式写入当前 Laravel staging；`missing/breached/retired` 不得进入 staging 或触发重签；
...
1. 仅从 `result_integrity_state=available` 且刚完成路径/SHA/size 复核的 Java result 或当前 worker 的确定性非签名输出写入 staging；
...
并在同一事务将 operation/request/workflow 投影为 `manual_review`、CAS appearance `evidence_hold_state=active` 并写 hold reason/start。V1 不提供双人裁决 UI，由受控管理员脚本和审计记录处理；隔离文件永不公开或作为 workflow base。
...
V1 不建设双人裁决产品，但必须提供受控 CLI/后台命令，用于 `manual_review` 和 `irreversible_failed` 的证据结案：
...
2. 若 Java execution=completed、`result_integrity_state=available` 且 result SHA/size/validation 全部匹配，管理事务记录 `resolution=adopt_completed`，CAS operation `manual_review→processing`、stage=`java_polling`，清旧 lease并写专用 `ResumePdfOperationFromJavaResult` outbox；后续 worker 领取新 lease/epoch，只允许 GET 同一 Java result 并继续 staging/事务 B，禁止进入签名 POST 分支。appearance hold 保持 active，直到事务 B 成功后再释放；
...
4. 若 Java execution=`failed_after_private_key_known`，核对稳定错误证据、确认不存在 promoted/ready 可用结果，投影并保持 operation=`irreversible_failed`、workflow/request=`failed`，清 active operation/workflow；不得伪装为 outcome unknown、completed 或同 operation 可重试；
5. 若 manual review 最终能证明未调用私钥、没有 CMS/TSA/result 字节且没有 promoted 文件，CAS operation `manual_review→failed`，清 active operation，将原 request 恢复 `available`、workflow 恢复 `signing`；原 challenge 已消费且原 appearance 结案后释放/隔离，用户必须创建新 appearance、challenge 和 operation，但可以继续同一 workflow，无需新 generation。若 Java execution 先前已经记为 `outcome_unknown`，该行保持历史终态不重分类，管理员 resolution event 明确记录“后续证据证明未越过边界”；新 operation 使用新的 UUID，不能复用旧 execution；
6. 若 Java state=`outcome_unknown` 且仍无法证明结果归属，继续保持 manual review。只有双人或受控管理员裁决形成 `confirmed_no_usable_result`，并确认不存在可采用的 completed/promoted/ready 结果后，才可把 operation 投影为 `irreversible_failed`、request/workflow=`failed`、清 active operation/workflow；后续重做必须从可信父 revision 创建新 generation，绝不能按第 5 条恢复同一 request；
7. terminal 结案事务写 appearance `hold_released_at`、CAS `evidence_hold_state=active→none` 和新的 `retention_until`；采用 completed result 时由成功事务 B 执行释放。sweeper 在释放前不得删除；
8. 所有 manual-review 状态迁移均要求管理员 resolution fingerprint 和完整审计；任何处置都不得把 quarantine 文件直接改名为 published revision。
...
| Java completed + result `retiring` | 只允许授权 sweeper 持有；按 result 文件是否仍存在恢复删除或补交 retired，不进入签署恢复、不触发重签 |
...
| orphan file | 移入 quarantine，禁止自动登记 |
...
3. 确认目标字段预存在、未填、未锁定；
4. 只生成目标字段的可视 AP；
5. 创建一个 signature dictionary；
6. 使用 `saveIncrementalForExternalSigning(...)`；
7. 生成 PAdES-B-T CMS 和 RFC 3161 signature timestamp；
8. 写入 `/Contents`；
9. 执行分层验证；
10. 返回二进制 PDF 和结构化验证摘要。
...
- 目标签名 field `/V`、目标 widget/AP 和必要 AcroForm key 是唯一允许变化；
- 页面内容、Pages tree、Catalog、元数据和非目标字段不变；
...
| `GET /internal/pdf/signatures/executions/{operationUuid}/result` | 仅 completed + result available 且即时 SHA/size 复核通过时流式返回同一结果；retiring 返回 `409 EXECUTION_RESULT_RETIRING`，missing/breached 返回 `503 EXECUTION_RESULT_INTEGRITY_ERROR`，retired 返回 `410 EXECUTION_RESULT_RETIRED` |
...
同时实现最小 documents/revisions/workflow/request/field/slot/appearance/challenge/operation/outbox 和 Java execution ledger，并完成 pre-key 重试、post-key known failure、registration/execution deadline polling、outcome-unknown/manual-review、取消-vs-private-key 竞争及 completed-result missing/breached 六类故障测试。operation 直接冻结 policy/config/certificate/appearance snapshots。Java 使用最小权限数据库账号：只授予 `pdf_signing_operations` 白名单列 SELECT 和专用 `java_gate_version` 列级 UPDATE、immutable policy 表 SELECT、execution 表 SELECT/INSERT/UPDATE、execution-event 表 INSERT；Java 通过 gate CAS 获取 operation 行锁，不读取或修改 workflow/request/document/revision/publication。workflow 取消与 pointer 清理由 Laravel 的 5.14 集中仲裁服务负责。
...
- manual-review 运维流程与 appearance legal-hold/release sweeper；
- 真实 Chrome 完整流程。
...
- completed execution 的 result path/SHA/size 每次取回前重验；missing/breached 不回退 execution、不触发重签；存在 verified staging/promoted 副本时继续使用该副本，否则按正式 revision 是否已物化进入 manual review 或 append-only 完整性事件；result 退休必须由 Laravel 先冻结可验证 authorization snapshot，Java 不读取 revision/document 推导删除资格；
...
- 13 页、20 MB、三次签署和一次首页章在生产资源门槛内完成；
...
本版在 v13 Final Lean 基础上执行了“必要性、可达性、恢复性、首发价值、不可逆边界、取消竞态、结果完整性与最小权限”八类反向审查。
...
- completed execution 与 result bytes integrity 分层；available 才可采用，missing/breached 不回退 completed、不重签，retiring/retired 只来自崩溃可恢复的授权保留期清理；
...
| P1 manual-review 笔迹缺少保留字段 | appearance 增加 evidence hold reason/start/release、legal hold 与 deleted 字段；终态事务设置 hold，结案后才 release + 24h；sweeper 以 hold/reference 条件 fail closed | 故障测试覆盖结案前 sweep、结案后删除和处置失败不释放 |
...
v14 额外反向检查确认：所有 operation 创建后的状态推进都使用统一锁序；operation cancelled 是稳定幂等终态，取消事务同时关闭 outbox，且已处于 retryable `failed_before_private_key` 的 execution 也会关闭 retry；completed execution 的 result integrity 异常不会被误分类为 outcome unknown，也不会复活私钥调用；verified downstream 副本可继续完成提交；retention 先以 execution completion 计时、事务 B 再延长到 operation completion；`available→retiring→retired` 使删除中崩溃不会被误报为文件丢失；Java 只额外读取 immutable policy 表，算法/TSA/key 不由请求自由传入；报告编号 normalizer、late-seal source-field identity、NULL-safe publication CAS、persistent result volume 与 HMAC nonce/body deadline 均已形成唯一合同。

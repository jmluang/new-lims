      ...
      <section className="mx-auto max-w-[210mm] overflow-hidden border border-slate-500 bg-white text-slate-900 shadow-sm">
        ...
        <div className="border-t border-slate-500">
          ...
          <div className="grid grid-cols-[7rem_1fr_7rem_1fr]">
            ...
            <CellLabel>合同编号</CellLabel><Cell><TextCell form={form} editable={editable} name={'contract_no'} /></Cell>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[38rem] border-collapse text-sm">
            <thead><tr className="bg-slate-100"><TableHead className="w-[60%]">标准号及版本</TableHead><TableHead>资质要求</TableHead><TableHead>报告语言</TableHead>{editable ? <TableHead className="w-12">操作</TableHead> : null}</tr></thead>
            <tbody>
        ...
        <div className="grid grid-cols-[7rem_1fr_7rem_1fr] border-t border-slate-500 text-sm">
        ...
        {samples.fields.map((row, index) => (
          <div className="border-b border-slate-500" key={row.id}>
            <div className="grid grid-cols-[6rem_1fr_6rem_1fr_6rem_1fr]">
              ...
              <CellLabel>状态</CellLabel><Cell><SelectCell form={form} editable={editable} name={`samples.${index}.sample_condition`} options={sampleConditionOptions.map((option) => ({ value: option, label: option === 'good' ? '完好' : '异常' }))} /></Cell>
            </div>
            <div className="grid grid-cols-[6rem_1fr_6rem_1fr_6rem_1fr]">
              ...
              <CellLabel>样品数量</CellLabel><Cell>{editable
                ? <div className="flex"><input className={cellInput} type="number" min={1} {...form.register(`samples.${index}.quantity`, { valueAsNumber: true })} /><input className="w-12 border-l border-slate-300 px-1 text-center text-sm" {...form.register(`samples.${index}.quantity_unit`)} /></div>
            ...
            <div className="grid grid-cols-[6rem_1fr_6rem_1fr_6rem_1fr]">
              ...
              <CellLabel>异常说明</CellLabel><Cell><TextCell form={form} editable={editable} name={`samples.${index}.sample_condition_note`} /></Cell>
            </div>
        ...
        <div className="grid grid-cols-[7rem_1fr] border-b border-slate-500"><CellLabel>实验室名称</CellLabel><Cell><TextCell form={form} editable={editable} name={'address_lab_name'} /></Cell></div>
        <div className="grid grid-cols-[7rem_1fr] border-b border-slate-500"><CellLabel>实验室地址</CellLabel><Cell><AreaCell form={form} editable={editable} name={'address_detail'} minHeight="min-h-16" /></Cell></div>
        <div className="grid grid-cols-[7rem_1fr] border-b border-slate-500"><CellLabel>联系人</CellLabel><Cell><TextCell form={form} editable={editable} name={'address_contact'} /></Cell></div>
        <div className="grid grid-cols-[7rem_1fr] border-b border-slate-500"><CellLabel>联系电话</CellLabel><Cell><TextCell form={form} editable={editable} name={'address_phone'} /></Cell></div>
        <div className="grid grid-cols-[7rem_1fr]"><CellLabel>特别说明</CellLabel><Cell><AreaCell form={form} editable={editable} name={'shipping_notes'} minHeight="min-h-20" /></Cell></div>

        <div className="border-t border-slate-500 p-4"><div className="grid grid-cols-[1fr_15rem] items-center gap-4"><p className="text-base font-medium">委托单位声明：上述提供资料正确无误！</p><label className="text-sm">委托人（客户）签字{editable ? <input className="mt-1 w-full border-b border-slate-500 px-2 py-1 outline-none" {...form.register('client_signature')} /> : <div className="mt-1 min-h-8 border-b border-slate-500 px-2 py-1">{form.watch('client_signature')}</div>}</label></div><div className="mt-3 flex justify-end"><label className="text-sm">日期{editable ? <input type="date" className="ml-2 border-b border-slate-500 px-2 py-1 outline-none" {...form.register('client_sign_date')} /> : <span className="ml-2 inline-block min-w-28 border-b border-slate-500 px-2 py-1">{form.watch('client_sign_date')}</span>}</label></div></div>
...
function PartyRows({ form, editable, prefix, title }: { form: UseFormReturn<TestOrderFormValues>; editable: boolean; prefix: 'client' | 'manufacturer' | 'maker'; title: string }) {
  return <div className="border-t border-slate-500 text-sm"><div className="grid grid-cols-[7rem_1fr_4rem_1fr_3rem_1fr]"><CellLabel>{title}</CellLabel><Cell><TextCell form={form} editable={editable} name={`${prefix}_company`} /></Cell><CellLabel>联系人</CellLabel><Cell><TextCell form={form} editable={editable} name={`${prefix}_contact`} /></Cell><CellLabel>电话</CellLabel><Cell><TextCell form={form} editable={editable} name={`${prefix}_phone`} /></Cell></div><div className="grid grid-cols-[7rem_1fr_3rem_1fr]"><CellLabel>地址</CellLabel><Cell><TextCell form={form} editable={editable} name={`${prefix}_address`} /></Cell><CellLabel>邮箱</CellLabel><Cell><TextCell form={form} editable={editable} name={`${prefix}_email`} type="email" /></Cell></div></div>
}

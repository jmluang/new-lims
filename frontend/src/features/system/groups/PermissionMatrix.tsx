  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="text-xs text-slate-500">已选择 {selected.size} 项权限</div>
            ...
            <div className="p-3">
              ...
              {Object.keys(config.fields).length > 0 ? (
                <div className="mt-4 space-y-2">
                  <div className="text-xs font-medium text-slate-500">字段权限</div>
                  {Object.entries(config.fields).map(([field, actions]) => (
                    <div className="grid gap-2 rounded-md border border-slate-100 p-2 md:grid-cols-[160px_minmax(0,1fr)]" key={field}>
                      <div className="text-sm font-medium text-slate-700">{permissionFieldLabel(field)}</div>
